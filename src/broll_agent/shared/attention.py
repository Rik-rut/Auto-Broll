...
def get_attention_modes():
    ...
    if memory_efficient_attention is not None:
        ret.append("xformers")
    if sageattn_varlen_wrapper is not None:
        ret.append("sage")
    return ret
...
def pay_attention(
    ...
    if attention_mask is not None:
        requested_attn = offload.shared_state["_attention"] if force_attention is None else force_attention
        if _is_mps or requested_attn != "sdpa":
    ...
    attn = offload.shared_state["_attention"] if force_attention is None else force_attention
    ...
    if b > 1 and k_lens is not None and attn in ("sage", "sdpa"):
        ...
        if len(chunk_sizes) > 1 or k_lens[0] != k.shape[1]:
            ...
            k_chunks = [ u[:, :sz] for u, sz in zip(k_chunks, k_sizes, strict=False)]
            v_chunks = [ u[:, :sz] for u, sz in zip(v_chunks, k_sizes, strict=False)]
            o = []
            for sub_q, sub_k, sub_v in zip(q_chunks, k_chunks, v_chunks, strict=False): 
            ...
            o = torch.cat(o, dim = 0)
            return o
    elif (q_lens is not None or k_lens is not None) and attn in ("sage", "sdpa"):
        assert b == 1
        szq = q_lens[0].item() if q_lens is not None else lq
        szk = k_lens[0].item() if k_lens is not None else lk
    ...
    # output
        ...
        elif k_lens is not None and q_lens is None:

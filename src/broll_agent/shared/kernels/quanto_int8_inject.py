...
def set_kernel_debug(enabled: bool | None = None) -> None:
...
def _startup_status(enabled: bool, detail: str) -> None:
    ...
    if enabled:
        _log("Injected int8 kernels ACTIVE (backend=triton).")
    else:
...
def _add_bias_in_place_or_fallback(output: torch.Tensor, bias: torch.Tensor | None) -> torch.Tensor:
    if bias is None:
        return output
...
def _expand_scale_to_rows(scale: torch.Tensor, rows: int, dtype: torch.dtype, device: torch.device | None = None) -> torch.Tensor:
...
def _probe_triton_backend() -> tuple[object | None, str]:
    try:
        mod = importlib.import_module("broll_agent.shared.kernels.quanto_int8_triton")
    except Exception as exc:
        return None, f"failed to import broll_agent.shared.kernels.quanto_int8_triton ({exc})"

    if not hasattr(mod, "is_available"):
        return None, "broll_agent.shared.kernels.quanto_int8_triton.is_available() missing"
    try:
...
def _int8_linear_forward_triton_dense_fast(ctx, input: torch.Tensor, other: torch.Tensor, bias: torch.Tensor | None):
    ctx.save_for_backward(input, other)
    if _TRITON_MODULE is None:
        raise RuntimeError("Triton backend not initialized")
    _mark_kernel_used()
...
def _int8_linear_forward_triton(ctx, input: torch.Tensor, other: torch.Tensor, bias: torch.Tensor | None):
    ctx.save_for_backward(input, other)
    if _TRITON_MODULE is None:
        raise RuntimeError("Triton backend not initialized")
    _mark_kernel_used()
...
def disable_quanto_int8_kernel(notify_disabled = False) -> bool:
    ...
    _reset_runtime_state()
    if notify_disabled:
        _startup_status(False, "disabled by User.")
    return True


def maybe_enable_quanto_int8_kernel(verbose_level: int | None = None) -> bool:

...
class TextEncoderCache:
    ...
    def encode(
        ...
        if not parallel:
            results: list[Any] = []
            for prompt, cache_key in zip(prompts_list, keys_list, strict=False):
        ...
        for idx, (prompt, cache_key) in enumerate(zip(prompts_list, keys_list, strict=False)):
            cached = self._entries.get(cache_key)
            if cached is None:
        ...
        if missing_prompts:
            ...
            for cache_key, idx, encoded in zip(
                missing_keys, missing_indices, encoded_batch, strict=False
            ):
